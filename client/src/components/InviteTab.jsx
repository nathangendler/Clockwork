...
export default function InviteTab() {
    ...
    fetch("/api/events/create", {
      ...
      .then((result) => {
        if (result.ok) {
          ...
          if (
            result.status === 409 &&
            result.data?.error === "no_valid_slots"
          ) {
  ...
  return (
    <div>
      <h2>Invite Contacts to Meeting</h2>
      ...
      <form onSubmit={handleCreate} className="meeting-form">
        ...
        <div className="urgency-toggle">
          {["low", "normal", "high"].map((level) => (
            <button
              key={level}
              type="button"
              className={`urgency-button ${urgency === level ? "active" : ""}`}
        ...
        {selected.length === 0 && (
          <p className="form-hint">
            Invite at least one person to schedule a meeting.
          </p>
        )}
